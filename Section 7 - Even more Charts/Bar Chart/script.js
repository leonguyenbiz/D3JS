async function draw(){
    // Loading data
    const dataset = await d3.csv('/Bar Chart/data/data.csv', (d, index, columns) => {
        // d['below_10'] = parseInt(d['<10'])
        // d['between_10-19'] = parseInt(d['10-19'])
        d3.autoType(d)
  ...
  const stackData = stackGenerator(dataset).map((ageGroups) => {
    ageGroups.forEach((state) => {
      state.key = ageGroups.key
    })
